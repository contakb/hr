...
app.post('/loginUser', (req, res) => {
  ...
  // Query the PostgreSQL database to check the user's credentials
  const query = 'SELECT email, username FROM public.users WHERE email = $1 OR username = $2';
  ...
  pool.query(query, values, (error, result) => {
    ...
    console.log('Retrieved username and email:', storedUsername, storedEmail);
    console.log('Input username or email:', usernameOrEmail);

    // Perform the comparison outside the callback
    if (storedUsername === usernameOrEmail || storedEmail === usernameOrEmail) {
      // User authenticated successfully
      console.log('Authentication successful:', usernameOrEmail);
      res.json({ message: 'Logged in successfully', username: storedUsername });
    } else {
...
app.post('/company', (req, res) => {
  const { companyname, address, taxid, userid } = req.body;
